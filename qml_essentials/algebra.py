...
def lie_closure_paulis(
    generators: Sequence[Union[str, PauliWord]],
    ...
    Args:
        ...
        max_dim: Optional cap (at least 1) on the number of words.  Growth stops
            as soon as the basis holds ``max_dim`` words, which keeps an ansatz
            that saturates :math:`\mathfrak{su}(2^n)` from enumerating all
            :math:`4^n - 1` of them.  A result of length ``max_dim`` therefore
            means :math:`\dim \mathfrak{g} \geq` ``max_dim`` and the basis may be
            partial: a truncated basis no longer spans an algebra, so use it for
            the dimension only, not for :func:`g_purity_from_basis`.

    Returns:
        The list of Pauli words spanning :math:`i \mathfrak{g}`, truncated to
        ``max_dim`` entries if the cap was reached.
    """
    ...
    limit = float("inf") if max_dim is None else max_dim
...
def lie_closure_matrices(
    gens: Sequence[np.ndarray], tol: float = 1e-9
) -> List[np.ndarray]:
...
def g_purity_from_basis(
    state: np.ndarray,
    basis: Sequence[Union[str, PauliWord]],
) -> float:
...
def g_purity_matrix(
    state: np.ndarray,
    basis_mats: Sequence[np.ndarray],
) -> float:
