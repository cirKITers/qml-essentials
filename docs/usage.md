...
```python
fig = model.draw(figure="tikz", inputs_symbol="x", gate_values=False)
...
Inputs are represented with "x" by default, which can be changed by adjusting the optional parameter `inputs_symbol`.
