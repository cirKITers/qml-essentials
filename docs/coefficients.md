...
The Clifford commutation and Pauli bookkeeping are implemented symbolically on a stabilizer-tableau Pauli representation (`jaqsi.operations.PauliWord`), and the parameter-dependent coefficients are evaluated with vectorized operations.
...
The symbolic core is gate-set agnostic and lives in `jaqsi.operations`:
