...
from qml_essentials.ansaetze import Ansaetze, Block, Circuit, DeclarativeCircuit
...
def test_pulse_params_ansaetze() -> None:
    ...
    for ansatz, res in test_cases.items():
        ...
        try:
            res = model(pulse_params=model.pulse_params)
...
def test_pulse_benchmarks() -> None:
    ...
    _ = model(pulse_params=model.pulse_params)
    end = time.time()
